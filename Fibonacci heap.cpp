...
struct fibonacci_heap_head{
    fibonacci_heap_node* min;//指向的元素即为根链表中最小的元素
    int n;//堆总结点数
    fibonacci_heap_head(){
        n=0;
        min=NULL;
...
fibonacci_heap_node* fib_heap_insert(fibonacci_heap_head* H,double k);
...
fibonacci_heap_node* fib_heap_insert(fibonacci_heap_head* H,double k){//外部只需要提供一个key值即可，其他的均不用管
...
void fib_heap_insert(fibonacci_heap_head* H,fibonacci_heap_node* x){//外部需要申请节点x，并对x的key做修改，其他的均不用管
    x->degree=0;
    x->parent=NULL;
    x->child=NULL;
    x->mark=false;
...
fibonacci_heap_node* fib_heap_extract_min(fibonacci_heap_head* H){
    ...
    if(z!=NULL){
        ...
        //特殊情况处理
            ...
            H->min=c;
            // H->min=z->right;
            // consolidate_with_sentinel(H);//此时传进去的H->n还未改变，比实际的大1
        }else if(z!=z->right&&c==NULL){
...
void consolidate_with_sentinel(fibonacci_heap_head* H){//和算法导论上的不一样，区别主要在for each w的遍历上和最后根链表的整理
    ...
    int d;
    fibonacci_heap_node *w,*x,*y,*temp;//循环不变量w始终为正在访问的节点的下一个，也就是恰好在边界外，x,y,temp均在边界内，边界即为处理边界
    fibonacci_heap_node* sentinel=fib_heap_insert(H,maximum);//在H->min左侧插入哨兵用于之后的循环
